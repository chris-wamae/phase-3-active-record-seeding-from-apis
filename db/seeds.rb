...
# make a request to the endpoint for the individual spell:
    response = RestClient.get "http://www.dnd5eapi.co/api/spells/#{spell}"
...
spell_hash = JSON.parse(response)
  ...
  Spell.create(
    ...
    description: spell_hash["desc"][0] # spell_hash["desc"] returns an array, so we need to access
    #the first index to get just a string of the description
  )
end
